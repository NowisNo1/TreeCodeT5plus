...
class T5LayerSelfAttention(nn.Module):
    ...
    def forward_core(
        ...
        hidden_states_filtered, seq_idx2graph_idx_filtered, \
        gat_edges_filtered, remain_node_idx, \
        attention_output_filtered = self.filter_remain_nodes(
            hidden_states,
            gat_edges,
            seq_idx2graph_idx,
            attention_output,
        ...
        outputs = (hidden_states_filtered,) + attention_output_filtered[1:]  # add attentions if we output them
        # if gat_edges is not None and gat_edges.shape[1] > 0 and gat_edges.max() >= hidden_states.shape[0]:
        #     print("")
        return outputs, seq_idx2graph_idx_filtered, gat_edges_filtered, remain_node_idx
    ...
    def filter_remain_nodes(
        ...
        seq_idx2graph_idx_filtered = []
        gat_edges_filtered = []
        attention_output_new_first_filtered = []
        attention_output_new_other_filtered = []
        for batch_idx, batch_len in enumerate(batch_sizes):
            batch_h = hidden_states[ptr: ptr + batch_len]
            batch_seq2graph = seq_idx2graph_idx[ptr: ptr + batch_len]
            batch_gat_edges = gat_edges[batch_idx] if gat_edges is not None else None
            batch_attn_output = (
                attention_output[0][ptr: ptr + batch_len],
                attention_output[1][ptr: ptr + batch_len] if len(attention_output) > 1 else None
            )
            batch_remain_idx = []

            if self.gat_layer is not None and batch_gat_edges is not None and batch_gat_edges.shape[1] != 0:
                gat_output, attn = self.gat_layer(batch_h, batch_gat_edges)
                if self.layer_idx % 2 == 1:
                    rate = (1 - self.layer_idx / 10)
                    edge_index, alpha = attn
                    alpha = alpha.reshape(-1)[:batch_gat_edges.shape[1]]
                    total = alpha.numel()
                    ...
                    if len(indices) == 0:
                        ...
                        gat_edges_new = batch_gat_edges.transpose(0, 1)[indices].transpose(0, 1).to(device)

                    max_graph_node = batch_seq2graph.shape[0]
                    components = connected_components(gat_edges_new, num_nodes=max_graph_node)
                    target_components_idx = components[0]
                    graph_connected_nodes = torch.where(components == target_components_idx)[0]

                    is_in_connected = torch.zeros(max_graph_node, dtype=torch.bool, device=device)
                    is_in_connected[graph_connected_nodes] = True
                    valid_seq_indices = torch.where(is_in_connected[batch_seq2graph])[0]
                    valid_seq2graph = batch_seq2graph[valid_seq_indices]

                    graph_node_to_new_id = torch.full((max_graph_node,), -1, dtype=torch.long, device=device)
                    graph_node_to_new_id[graph_connected_nodes] = torch.arange(len(graph_connected_nodes),
                                                                               device=device)
                    idx2idx = graph_node_to_new_id[valid_seq2graph]

                    hidden_states_new = batch_h[valid_seq_indices]
                    attention_output_new_first = batch_attn_output[0][valid_seq_indices]
                    attention_output_new_other = batch_attn_output[1][valid_seq_indices] \
                        if batch_attn_output[1] is not None else None
                    gat_output_new = gat_output[valid_seq_indices]
                    ...
                    batch_remain_idx = ptr + valid_seq_indices
                    batch_seq2graph = idx2idx
                    batch_gat_edges = gat_edges_new
                    batch_h = hidden_states_new
                    batch_attn_output = (attention_output_new_first,
                                         attention_output_new_other if attention_output_new_other is not None else None)
                    gat_output = gat_output_new
                else:
                    batch_remain_idx = ptr + torch.arange(batch_seq2graph.shape[0], device=device)

                batch_h = self.fusion(batch_h + self.dropout(batch_attn_output[0]), gat_output)
            else:
                if batch_seq2graph is not None:
                    batch_remain_idx = ptr + torch.arange(batch_seq2graph.shape[0], device=device)
                batch_h = batch_h + self.dropout(batch_attn_output[0])

            remain_node_idx.append(batch_remain_idx)
            hidden_states_filtered.append(batch_h)
            gat_edges_filtered.append(batch_gat_edges)
            seq_idx2graph_idx_filtered.append(batch_seq2graph)
            attention_output_new_first_filtered.append(batch_attn_output[0])
            attention_output_new_other_filtered.append(batch_attn_output[1] if len(attention_output) > 1 else None)
            ptr += batch_len

        hidden_states_filtered = torch.cat(hidden_states_filtered, dim=0)
        seq_idx2graph_idx_filtered = torch.cat(seq_idx2graph_idx_filtered, dim=0)

        remain_node_idx = torch.cat(remain_node_idx, dim=0)

        attention_output_filtered = (
            torch.cat(attention_output_new_first_filtered, dim=0),
            torch.cat(attention_output_new_other_filtered, dim=0)
        )
...
class T5TreeBlock(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        if gat_edges is None:
            ...
            all_outputs = self.layer[0].forward_core(
            ...
            all_self_attn_outputs, seq_idx2graph_idx, gat_edges, all_remain_idx = all_outputs
            batch_sizes_new = []
            ptr = 0
            ...
            hidden_states = all_self_attn_outputs[0]
            attention_mask = None if attention_mask is None else attention_mask[all_remain_idx]
            position_bias = None if position_bias is None else position_bias[all_remain_idx]
            layer_head_mask = None if layer_head_mask is None else layer_head_mask[all_remain_idx]
            past_key_values = None if past_key_values is None else past_key_values[all_remain_idx]
            attention_outputs = (all_self_attn_outputs[1],)
