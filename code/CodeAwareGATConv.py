...
def orthogonal_with_var(tensor, var=0.01):
    init.orthogonal_(tensor, gain=1.0)
    current_var = tensor.var()
    scale = torch.sqrt(var / current_var)
    tensor.data.mul_(scale)


class CodeAwareGATConv(MessagePassing):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        # add some operation
        # orthogonal_with_var(self.att_src, var=0.1)
        # orthogonal_with_var(self.att_dst, var=0.1)
